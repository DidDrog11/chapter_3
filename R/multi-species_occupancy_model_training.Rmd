...
The same syntax is used for `msPGOcc()` functions as for single-species models. For multi-species models, the multi-species detection-nondetection data stored in `y` is now a three-dimensional array with domensions corresponding to species, sites and replicates. 
...
These covariates can only vary by site, thus, occurrence covariates are formated where the rows represent the sites and the columns represent the covariates. We will only use elevation in this example. 
...
Coordinates should be formatted as a matrix with $J$ rows (wheree $J$ is the number of sites) and two columns, with the horizontal component in the first column and the vertical in the second. `spOccupancy` assumes coordinates are projected. So do not provide as latitude and longitude. The coordinates below are in UTM Zone 19N.
